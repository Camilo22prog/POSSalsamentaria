-- =============================================================
-- update_v1.1.0_cumulative.sql
-- Actualizacion acumulativa desde v1.0.0 o v1.0.1 -> v1.1.0
-- Combina las actualizaciones de v1.0.1 y v1.1.0 de forma idempotente.
-- =============================================================

USE POSSalsamentaria;
GO

-- Asegurar que la columna AppliedDate en __DatabaseVersion permita NULLs
IF EXISTS (SELECT 1 FROM sys.tables WHERE name = '__DatabaseVersion')
BEGIN
    IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('__DatabaseVersion') AND name = 'AppliedDate')
    BEGIN
        ALTER TABLE __DatabaseVersion ADD AppliedDate DATETIME NULL;
    END
    ELSE
    BEGIN
        ALTER TABLE __DatabaseVersion ALTER COLUMN AppliedDate DATETIME NULL;
    END
END
GO
...
BEGIN TRY
    ...
    IF NOT EXISTS (SELECT 1 FROM __DatabaseVersion WHERE Version = '1.0.1')
    BEGIN
        INSERT INTO __DatabaseVersion (Version, Description)
        VALUES ('1.0.1', 'Actualizacion acumulativa v1.0.1: IVA, metodos de pago, arqueo, retiros, clientes, venta por peso.');
    END
...
BEGIN TRY
    ...
    -- ============================================================
    IF NOT EXISTS (SELECT 1 FROM __DatabaseVersion WHERE Version = '1.1.0')
    BEGIN
        INSERT INTO __DatabaseVersion (Version, Description, AppliedDate)
        VALUES ('1.1.0', 'Actualizacion v1.1.0: Modulo de Gastos Operativos y Configuracion de Sistema.', GETDATE());
    END
